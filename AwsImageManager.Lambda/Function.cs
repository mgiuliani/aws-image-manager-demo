...
namespace AwsImageManager_Lambda
{
    public class Function
    {
        ...
        public async Task FunctionHandler(S3Event input, ILambdaContext context)
        {
            foreach(var record in input.Records)
            {
                ...
                ImageItem dbRecord = null;
                using (var dbContext = new DynamoDBContext(this.DynamoDBClient))
                {
                    var results = await dbContext.QueryAsync<ImageItem>(record.S3.Object.Key).GetRemainingAsync();
                    if (results?.Count > 0)
                    {
                        dbRecord = results.First();
                    }

                    if (dbRecord == null)
                    {
                        Console.WriteLine("No records found in DynamoDB, exiting.");
                    }
                    ...
                    dbRecord.tags = detectResponses.Labels.Select(l => new ImageTag()
                    {
                        Name = l.Name,
                    ...
                    await dbContext.SaveAsync(dbRecord);
                }
            }
            return;
        }
